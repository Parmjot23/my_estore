...
import { Eye, Edit3, Trash2, PlusCircle, ShoppingBag, LogOut, UserCircle2, KeyRound, Save, Menu as MenuIcon } from 'lucide-react';
...
const AccountDashboard = () => {
  ...
  const handleSaveAddress = async (addressDataToSave) => {
    try {
      let savedAddress;
      ...
      <section className="overflow-hidden py-10 md:py-16 lg:py-20 bg-gray-100 dark:bg-dark-2">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="bg-white dark:bg-dark shadow-lg rounded-xl">
              <header className="h-[72px] bg-white flex justify-between items-center px-4 sm:px-6 shadow-sm rounded-t-xl">
                ...
                <nav aria-label="Account navigation" className="flex items-center">
                    ...
                    {navLinks.map((tab) => (
                      <li key={tab.id}>
                        <button
                          ...
                          className={`flex items-center gap-2 px-2 py-2 text-sm font-medium border-b-2 transition-colors duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${activeTab === tab.id ? 'text-indigo-600 border-indigo-600' : 'border-transparent text-gray-700 dark:text-gray-300 hover:text-gray-900'} ${tab.id === 'logout' ? 'hover:text-red-600' : ''}`}
                        >
                          {tab.icon}
                          {tab.label}
                        </button>
                  ...
                  <Menu as="div" className="relative md:hidden">
                    <Menu.Button className="p-2 text-gray-600 hover:text-gray-900 transition-colors duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500">
                      <MenuIcon size={24} />
                    </Menu.Button>
                    <Menu.Items className="absolute right-0 mt-2 w-48 origin-top-right bg-white shadow-lg rounded-md ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                      {navLinks.map((tab) => (
                        <Menu.Item key={tab.id}>
                          {({ active }) => (
                            <button
                              onClick={() => { tab.id === 'logout' ? handleLogout() : setActiveTab(tab.id); }}
                              className={`w-full flex items-center gap-2 px-4 py-2 text-sm transition-colors duration-150 ${active ? 'bg-gray-50' : ''} ${activeTab === tab.id ? 'text-indigo-600' : 'text-gray-700 hover:text-gray-900'} ${tab.id === 'logout' ? 'hover:text-red-600' : ''}`}
                            >
                              {tab.icon}
                              {tab.label}
                            </button>
              ...
              <div>
                {/* Dashboard */}
                <div className={`${activeTab === "dashboard" ? "block" : "hidden"} p-6 md:p-10`}>
                ...
                {/* Orders */}
                <div className={`${activeTab === "orders" ? "block" : "hidden"} border-t border-gray-200 dark:border-dark-3`}>
                   <h3 className="text-2xl font-semibold text-dark dark:text-white mb-0 p-6 md:p-10 pb-4">My Orders</h3>
                  <Orders />
                </div>

                {/* Addresses */}
                <div className={`${activeTab === "addresses" ? "block" : "hidden"} p-6 md:p-10 border-t border-gray-200 dark:border-dark-3`}>
                ...
                <div className={`${activeTab === "account-details" ? "block" : "hidden"} p-6 md:p-10 border-t border-gray-200 dark:border-dark-3`}>
                <div className="mb-10">
