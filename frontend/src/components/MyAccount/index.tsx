...
const MyAccount = () => {
  ...
  }, [isAuthenticated, authUser, authIsLoading]);
  ...
  const handleUpdateDetails = async (e: FormEvent) => {
    ...
    try {
      // Exclude username and email from the direct update payload
      // as they are typically not updated this way or have special handling.
      const updatePayload = {
        first_name: userData.first_name,
        last_name: userData.last_name,
        // Add other updatable fields here if your backend UserSerializer allows them
      };
      ...
      toast.error(err.data?.detail || err.message || "Failed to update details.");
    } finally {
  ...
  const handleChangePassword = async (e: FormEvent) => {
    ...
    toast.info("Password change functionality needs backend implementation. This is a demo.");
  ...
  if (!authIsLoading && !isAuthenticated) {
    return (
         <>
            ...
            <section className="overflow-hidden py-20 bg-gray-100 dark:bg-dark-2">
                <div className="container mx-auto px-4 text-center">
                    <p>Please <Link href="/signin" className="text-blue hover:underline">sign in</Link> to view your account.</p>
                </div>
            </section>
        </>
    );
  ...
  if (!authUser) {
    return (
         <>
            ...
            <section className="overflow-hidden py-20 bg-gray-100 dark:bg-dark-2">
                <div className="container mx-auto px-4 text-center">
                    <p>Could not load user details. Please try again or <Link href="/signin" className="text-blue hover:underline">sign in</Link> again.</p>
                </div>
            </section>
        </>
    );
  ...
  return (
    <>
          ...
          <div className="flex flex-col xl:flex-row gap-8">
            ...
            <div className="xl:w-1/4 bg-white dark:bg-dark shadow-lg rounded-xl p-6 self-start">
              <div className="flex items-center gap-4 mb-8 pb-6 border-b border-gray-200 dark:border-dark-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 break-all">{authUser?.email}</p>
                </div>
              </div>
            ...
            <div className="xl:w-3/4">
              ...
              <div className={`${activeTab === "orders" ? "block" : "hidden"} bg-white dark:bg-dark shadow-lg rounded-xl`}>
                 <h3 className="text-2xl font-semibold text-dark dark:text-white mb-6 p-6 md:p-10 pb-4">My Orders</h3>
                <Orders />
              </div>
                        ...
                        {addresses.map((addr) => (
                            <div key={addr.id} className="border border-gray-200 dark:border-dark-3 rounded-xl p-6 bg-gray-50 dark:bg-dark-input shadow-sm hover:shadow-md transition-shadow">
                                <h4 className="font-semibold text-lg text-dark dark:text-white mb-2 capitalize flex justify-between items-center">
                                    ...
                                    {addr.is_default && <span className="ml-2 text-xs bg-green-100 text-green-700 px-2.5 py-1 rounded-full font-medium">Default</span>}
                                </h4>
              ...
              <div className={`${activeTab === "account-details" ? "block" : "hidden"} bg-white dark:bg-dark shadow-lg rounded-xl p-6 md:p-10`}>
                ...
                <form onSubmit={handleUpdateDetails} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6"> {/* Added gap-y-6 */}
                    <div>
                  ...
                  <div className="space-y-1.5"> {/* Added wrapper for spacing */}
                ...
                <form onSubmit={handleChangePassword} className="space-y-6">
                  <div>
                    ...
                    <input type="password" name="current_password" id="current_password" value={passwordData.current_password} onChange={handlePasswordChange} required className="form-input" autoComplete="current-password"/>
                  </div>
                  <div>
                    <label htmlFor="new_password" className="form-label">New Password</label>
                    <input type="password" name="new_password" id="new_password" value={passwordData.new_password} onChange={handlePasswordChange} required className="form-input" autoComplete="new-password"/>
                  </div>
                  <div>
                    <label htmlFor="confirm_new_password" className="form-label">Confirm New Password</label>
                    <input type="password" name="confirm_new_password" id="confirm_new_password" value={passwordData.confirm_new_password} onChange={handlePasswordChange} required className="form-input" autoComplete="new-password"/>
                  </div>
                  <div className="pt-2"> {/* Added padding top for button */}
      ...
      <style jsx>{`
        .form-label {
          @apply block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5; /* Increased bottom margin */
        }
        .form-input {
          @apply block w-full px-4 py-3 text-sm text-dark dark:text-white bg-gray-50 dark:bg-dark-input border border-gray-300 dark:border-dark-3 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors placeholder-gray-400 dark:placeholder-gray-500; /* Adjusted padding, bg, placeholder */
        }
        .form-input:read-only { /* Style for read-only inputs */
            @apply bg-gray-100 dark:bg-dark-3 cursor-not-allowed opacity-70;
        }
        .btn-primary {
          @apply inline-flex items-center justify-center px-6 py-3 bg-blue text-white text-sm font-semibold rounded-lg shadow-md hover:bg-blue-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-dark transition-all duration-150 ease-in-out disabled:opacity-70; /* Enhanced styling */
        }
        .btn-secondary {
            @apply inline-flex items-center justify-center px-6 py-3 bg-gray-200 text-gray-700 dark:bg-dark-3 dark:text-gray-300 text-sm font-semibold rounded-lg shadow-md hover:bg-gray-300 dark:hover:bg-dark-4 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 dark:focus:ring-offset-dark transition-all duration-150 ease-in-out; /* Enhanced styling */
        }
      `}</style>
    </>
  );
